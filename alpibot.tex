...
\author{Bianchi Luciano, Buzniak Esteban Alejandro, Ramele~Rodrigo,~\IEEEmembership{Member,~IEEE,}
        and Santos~Juan~Miguel% <-this % stops a space
\thanks{E. A. Buzniak, R. Ramele  and J.M.Santos are with the CiC Laboratory at the Department
...
% The paper headers
\markboth{IEEE Latin American Transactions}%
...
\begin{abstract}
Patients that suffer Chronic Obstructive Pulmonary Disease (COPD) undergo a procedure called Pulmonary Rehabilitation that helps them to improve disease prognosis.  During Pulmonary Rehabilitation procedures patients require oxygen assistance.  The oxygen tank cannot be carried by the patient and external assistance is required.   This work provides an initial study on the controlling mechanism of a differential tethered robot that implements a leader-follower configuration to carry the oxygen tank for Pulmonary Rehabilitation procedures.  Two alternative control strategies are proposed.  Results are simulated and tested on a real prototype in a Motion Capture System.
\end{abstract}


% Note that keywords are not normally used for peerreview papers.
\begin{IEEEkeywords}
SAR, IoRT, tethered, leader-follower, pulmonary rehabilitation, COPD
\end{IEEEkeywords}
...
To be effective, any technological solution for the medical community requires active involvement of key stakeholders: physicians, care-givers, patients and their families~\cite{Gaggioli2006,Fasola2012,Cherubini2008,Wolpaw2012BraincomputerIP,Salvini2015}.  Hence, design methodologies that allow rapid prototyping can bring quickly feedback from real users about drawbacks or opportunities for improvements. 
...
Encoders in each reel measure the difference in length of each thread compared to its initial position. This difference in length is the input for the control algorithm.  Using the encoder, the difference in length of the thread can be measured with Equation~\ref{eq:encoder_len}.

\begin{equation}
\label{eq:encoder_len}
L_d = pulses . \frac{2\pi r}{ppr}
\end{equation}{}

\noindent where the variable $r$ is the radius of the reel and $ppr$ is the pulses per revolution (resolution) of the encoder.  The initial position of the threads is configurable.
...
Frames are constructed from aluminum extrusions produced by Makeblock (Shenzhen, China).   The prototype is a three-wheeled robot with two frontal differential drive wheels and a free castor wheel as a third point of contact on the back, as can be seen on Figure~\ref{fig:alpibot2}.  

Two motors Makeblock Optical Encoder Motor-25 9V/86 rpm are used on in-wheel configuration providing optical encoding.  A microcontroller Arduino (Arduino LLC, Italy) Mega 2560 is used to implement the control loop and to provide encoder processing.  On top of it an Adafruit (Adafruit, New York City) Motor Shield v2 bridge is used to drive the four DC motors, one for each wheel and one on each reel.   The Arduino board is also connected to a Single Board Computer (SBC) Raspberry Pi (Raspberry Pi Foundation, United Kingdom) 3B+ through serial connection on the USB port.
...
The prototype can be seen on Figure~\ref{fig:alpibot1} and~\ref{fig:alpibot2}.  It has two separate batteries, one powering the Raspberry Pi, the Arduino board and the encoder electronics, and the other powering the drive and reel motors. The first battery is a commercial power bank with a capacity of 10000 mA$\cdot$h, and an output of 3.1A (over two USB ports) at 5V. The motor battery is a set of 10 AAA nickel–metal hydride batteries (1.5V each), with a total output of 15V.  

\subsection{Active Reel Spring}

As previously mentioned, an active spring mechanism is also put in place to keep the threads tense.  However, in order to extend the useful life of the reel motors, and to save battery, an algorithm to  activate and deactivate the motors is developed. 
...
\begin{equation}
v_{tar} = c_v (\frac{D_l + D_r}{2} - d_0)
\label{eq:lemniscata}
\end{equation}

\begin{equation}
vleft_{tar} = v_{tar} + c_{\alpha} (D_l - D_r)
\label{eq:lemniscata2}
\end{equation}

\begin{equation}
vright_{tar} = v_{tar} - c_{\alpha} (D_l - D_r)
\label{eq:lemniscata3}
\end{equation}

\noindent where  $c_v$ and $c_{\alpha}$ are constant coefficients used for calibration.  As shown in Figure~\ref{fig:follow_params}, reel encoder information is $D_l$ and $D_r$ for the left and right reel, and finally $vleft_{tar}$ are $vright_{tar}$ are the power values that are directly used to drive power to each wheel motor.

To stop the vehicle completely when it is close to its expected position, an additional condition is added: 

\begin{equation}
\textit{if  }{\frac{D_l + D_r}{2} < d_0}\textit{ then } vleft_{tar} = vright_{tar} = 0 
\end{equation}

\noindent where $d_0$ is a constant offset that is used to customized the length of the thread where the robot does not move.
...
\begin{figure}[]
    ...
    % Text Node
    \draw (390.58,1925.93) node [anchor=west] [inner sep=0.75pt]  [font=\normalsize,color={rgb, 255:red, 0; green, 0; blue, 0 }  ,opacity=1 ]  {$vleft_{tar}$};
    % Text Node
    \draw (407.58,2100.93) node [anchor=west] [inner sep=0.75pt]  [font=\normalsize,color={rgb, 255:red, 0; green, 0; blue, 0 }  ,opacity=1 ]  {$vright_{tar}$};
...
\item Go forward in a straight line until the vehicle is at the expected distance to the subject or the vehicle is not moving in the direction of the subject.
\end{itemize}

The procedure is detailed in Algorithm~\ref{alg:rotateandgoalg}.  The variable $V_r$ is the speed at which the vehicle will rotate on its axis, and $V_f$ is the speed at which the vehicle will move forward once it can move on the subject's direction.  The algorithm requires three parameters, $c_v$ and $c_r$ that regulates the coefficients for the forward and rotation movement, and an additional parameter $Dt_off$ that regulates the sensibility of the rotation movement.

\begin{algorithm}[]
\caption{Rotate and Go algorithm}
\begin{algorithmic}
\STATE $Dt \gets D_l - D_r$
\STATE $Dm \gets \frac{D_r+D_l}{2}$
\STATE $V_r \gets c_r * (abs(Dt) - Dt_off) + base_{vr}$ 
\STATE $V_f \gets c_v * (Dm - Dm_off)$

\IF{$abs(Dt) > Dt_{off}$}
\IF{$D_l > D_r$}
\STATE $vleft_{tar} \gets V_r$
\STATE $vright_{tar} \gets -V_r$
\ELSE
\STATE $vleft_{tar} \gets -V_r$
\STATE $vright_{tar} \gets V_r$
\ENDIF
...
\ELSE
\IF{$D_m > Dm_{off}$}
\STATE $vright_{tar} \gets V_f$
\STATE $vleft_{tar} \gets V_f$
\ELSE
\STATE $vright_{tar} \gets 0$
\STATE $vleft_{tar} \gets 0$
\ENDIF
\ENDIF
\STATE Return $vright_{tar}$ and $vleft_{tar}$.
\end{algorithmic}
...
Regarding metrics, three are proposed to evaluate the performance.  They are:

\begin{itemize}
    \item \textit{Normal path deviation, n.p.d.}: the subject trajectory is divided into small segments and then the normal distance to the robot trajectory is calculated for each of those segments. Path deviation is relevant to evaluate how closely the robot mimics the leader path, which is the ultimate goal of the robotic vehicle. 
    \item \textit{Total path deviation, t.p.d.}: The area under the curve resulting from the the \textit{normal path deviation} over the length traveled by the leader.
    \item \textit{Maximum path deviation, m.p.d.}: the maximum \textit{normal path deviation} registered during an experiment.    
    \item \textit{Robot-leader distance, r.l.d.}: The euclidean distance between the robot and the leader, at any point in time. This is particularly important since the robot has a limited amount of thread available, so if the leader uses all the available thread, it will start dragging the robot and damaging the following mechanis, and overall it may rise the possibility of disconnecting the breathing oxygen cannula. This is a scenario that must not happen under any circumstance, as it can also be dangerous for a potential patient using the device. 
\end{itemize}{}
...
A model of the proposed design was first built on Webots~\cite{Michel2004} simulator.  The threading mechanism was implemented using virtual threads~\cite{Rekleitis2001}.  The leader traveled according to a predefined path with constant velocity, followiing the lemniscate trajectory.
...
\caption{Hardware prototype on the motion capture system and a testing subject holding the threads.  The lemniscate of gerone was marked on the floor.  The subject follows this track on the experiments performed.}
...
Simulation results for both control strategies are shown on Figure~\ref{fig:simulationresults}.  Results metrics for the simulations are shown on Table~\ref{tab:simulationmetricsftt} for the Follow-The-Thread strategy, whereas metrics for Rotate-And-Go are shown on Table~\ref{tab:simulationmetricsrg}.


\begin{center}
\begin{tabular}{ |c|c|c|c| }
\hline
$c_v$ & $c_{\alpha}$ & m.p.d. & t.p.d. \\
\hline
10  &   15  & 0.3614 & 2.0651\\
15  &   5  & 0.4325 & 2.055\\
...
\hline
\end{tabular}
\captionof{table}{Max path deviation m.p.d. [m] and Total path deviation t.p.d. for different Follow the Thread constants.}
...
\hline
\end{tabular}
\captionof{table}{Max path deviation m.p.d. (in [m]) and total path deviation t.p.d. for different Rotate and Go constants.}
...
\begin{figure} 
    \centering
  \subfloat[ \label{1a}]{%
       \includegraphics[width=0.49\linewidth]{images/ft_a2_n1500_cv15_cr15.eps}}
    \hspace*{-1.5em}
  \subfloat[ \label{1b}]{%
        \includegraphics[width=0.49\linewidth]{images/rgv_cv20_cr20_dr10.eps}}
    \\
    \vspace*{-1.5em}
  \subfloat[ \label{1c}]{%
        \includegraphics[width=0.49\linewidth]{images/ft_cv15_cr10_leader_robot_speed.eps}}
    \hspace*{-1.5em}
  \subfloat[ \label{1d}]{%
        \includegraphics[width=0.49\linewidth]{images/rg_cv20_cr20_bv10_dtoff10_leader_robot_speed.eps}}
    \\
    \vspace*{-2.5em}
  \subfloat[ \label{1e}]{%
...
\begin{figure} 
    \centering
  \subfloat[ \label{1a}]{%
       \includegraphics[width=0.49\linewidth]{images/ft1cap.eps}}
    \hspace*{-1.5em}
  \subfloat[ \label{1b}]{%
        \includegraphics[width=0.49\linewidth]{images/rg2cap.eps}}
    \\
    \vspace*{-1.5em}
  \subfloat[ \label{1c}]{%
        \includegraphics[width=0.49\linewidth]{images/ft1cap_leader_robot_speed.eps}}
    \hspace*{-1.5em}
  \subfloat[ \label{1d}]{%
        \includegraphics[width=0.49\linewidth]{images/rg2cap_leader_robot_speed.eps}}
    \\
    \vspace*{-2.5em}
  \subfloat[ \label{1e}]{%
...
\begin{tabular}{ |c|c|c|c| }
\hline
$c_v$ & $c_{\alpha}$ & m.p.d. [m] & t.p.d. \\
\hline
25  &   20  & 0.3876 & 1.9761\\
25  &   35  & 0.4672 & 2.3528\\
\hline
\end{tabular}
\captionof{table}{Max path deviation m.p.d. and area under normal path deviation t.p.d. in motion capture experiments using Follow the Thread.}
...
\end{tabular}
\captionof{table}{Max path deviation m.p.d. and area under normal path deviation t.p.d. in motion capture experiments using Rotate and Go.}
...
From the leader and robot trajectories in Figure~\ref{fig:simulationresults}(a,c), both strategies exhibits basic following behaviour.  However, Rotate-And-Go on Figure~\ref{fig:simulationresults}(b,d) generates a more irregular path, due to the two stage movement algorithm.   
...
%From the leader and robot trajectories in Figure \ref{fig:sim_ft_cv15_cr15}, we can see that the robot using Follow the Thread control exhibits basic following behaviour. Deviation occurs when the leader path turns with a tight radius. Comparing different simulations using different values of $c_{\alpha}$, we can also see that the robot tends to miss or cut corners when the leader turns, depending on the value of this constant (figures \ref{fig:sim_ft_cv15_cr5}, \ref{fig:sim_ft_cv15_cr10} and \ref{fig:sim_ft_cv15_cr15}). When $c_{\alpha}$ is low, the robot is slow to turn and makes wider turns, and when it is high, it can turn sharply as soon as the leader starts turning, so it will "cut" the curve. 
...
%The rotateand go We see that this strategy also exhibits a following behaviour, although in a more irregular path, as expected, due to the two stage movement algorithm. In this case we evaluated the effects of the constants $c_v$, $c_r$ and $Dt_{off}$. 
...
%Both behaviours are described by figures \ref{fig:ft_speed_1} and \ref{fig:rg_speed_1}, which show the speed of the leader and the robot over time.
...
In line with the simulated results, a similar behaviour was found on the experiments performed inside the Motion Capture Lab, and the robot exhibits following behaviour for both control strategies as shown on Figure~\ref{fig:realworldresults}.   Parameters had to be readjusted for the real world case, but the same situation occurs for both strategies.   


%We can see that, again, the robot exhibits following behaviour. The trajectories in the figures mentioned above show a similar behaviour to the one in the software simulation, for both control strategies.
...
In the demonstration, the robot design was outlined and an explanation was given on how the robot worked, how it was built and how to operate it. Both control strategies were expounded, along with the main superficial differences between them. 

Afterwards, health care professionals were invited to use the robot themselves, simulating they were the patient being followed. They were allowed to switch between the two control strategies to evaluate both of them, and made different tests, one walking along standardized path marked on the floor and another one walking freely along the available space of the Motion Capture Lab. They used the robot freely to get a general idea of how it behaved, and how it could be used in the rehabilitation process. After all evaluations were finished, various aspects of the vehicle prototype were discussed and then a survey was handed out to document their experience with the robot, get their expert opinion on how the two strategies compared against each other, and what other improvements were needed in order to deliver a fully usable product.  Survey questions and their averaged numerical evaluations are provided in Table~\ref{tab:alpi_q_table_1}.
...
From the practical experiments, it is verified that the both algorithms gave reasonable following performance in the task of following the leader along a lemniscate-shaped path. This reasonable following behaviour is accomplished with a simple mechanism, a characteristic that significantly keep the price of the device low, putting it within reach of many medical institutions.
...
The next steps for this project is to scale the design to be able to carry safely the oxygen tank.

Further research is needed to develop a full fledged solution: 
\begin{itemize}
    ...
    \item Redesign the active spring mechanism in the reels to avoid overheating. In our tests, the small electric motors in each reel got very hot after just 20 minutes of use. The active reel algorithm is enough to keep them in manageable temperatures (under $70^0C$), which is enough for testing the prototype, but it will not be appropriate for a useful robot. Not only because the heat produced can be dangerous, but also because the motors can overheat and break their insulation in regular use.
