...
\thanks{E. Buzniak, R. Ramele  and J.M.Santos are with the CiC Laboratory at the Department
...
\begin{abstract}
Patients that suffer Chronic Obstructive Pulmonary Disease (COPD) underwent a procedure called Pulmonary Rehabilitation that helps them to improve disease prognosis.  During Pulmonary Rehabilitation procedures patients require external oxygen assistance.  The oxygen tank cannot be carried by the patient and external assistance is required.   This work provides an initial study on the controlling mechanism of a differential tethered robot that implements a leader-follower configuration to carry the oxygen tank for Pulmonary Rehabilitation procedures.  Two alternative control strategies are proposed.  Results are simulated and tested on a real prototype in a Motion Capture System.
\end{abstract}
...
Frames are constructed from aluminum extrusions produced by Makeblock (Shenzhen, China).  
...
Reels were designed from PVC extrusions based on figure.  They are glued to regular DC motors scavenged from old compact discs.  
...
\caption{Robot prototype built to evaluate the real world scenario of the following mechanisms for the tethered robot.}
...
A real world experiment was performed, pegging to the same conditions implemented on the simulation environment.  A motion capture system is used to track the movement of a human leader along a predetermined path.   The motion tracking system consists of an array of 16 OptiTrack (NaturalPoint Inc, Oregon, US) Flex 3 cameras, which measure the position of reflective markers with an accuracy of $\pm1$ a sampling rate of 100 Hz. The calibration and data collection was made using the Motive motion capture software. 

A tracked marker was placed in each side of the robot (on top of each thread reel). The human leader used his hand to grab the tip at which the two tethers were tied together. A third marker was placed in his hand, using a glove. The lemniscate of Gerono, used in the simulation, was drawn in the floor (\ref{fig:cap_lab_lemniscate}), and the human leader tried to move his hand following this shape as close as possible, with stable speed. 

The shape was drawn according to the shape described in equation \ref{eq:lemniscate_of_gerono}, using $a = 2$, in meters. 

The three markers allowed us to measure the trajectory of the robot and the leader, and then obtain the same metrics calculated in the simulation. 
...
\begin{figure} 
    \centering
  \subfloat[a\label{1a}]{%
       \includegraphics[width=0.45\linewidth]{images/ft_a2_n1500_cv15_cr15.eps}}
    \hfill
  \subfloat[b\label{1b}]{%
        \includegraphics[width=0.45\linewidth]{images/rgv_cv20_cr20_dr10.eps}}
    \\
  \subfloat[c\label{1c}]{%
        \includegraphics[width=0.45\linewidth]{images/ft_cv15_cr10_leader_robot_speed.eps}}
    \hfill
  \subfloat[d\label{1d}]{%
        \includegraphics[width=0.45\linewidth]{images/rg_cv20_cr20_bv10_dtoff10_leader_robot_speed.eps}}
    \\
  \subfloat[d\label{1e}]{%
        \includegraphics[width=0.85\linewidth]{images/ft_vs_rg_dist.eps}}
...
\begin{figure} 
    \centering
  \subfloat[a\label{1a}]{%
       \includegraphics[width=0.45\linewidth]{images/ft1cap.eps}}
    \hfill
  \subfloat[b\label{1b}]{%
        \includegraphics[width=0.45\linewidth]{images/rg2cap.eps}}
    \\
  \subfloat[c\label{1c}]{%
        \includegraphics[width=0.45\linewidth]{images/ft1cap_leader_robot_speed.eps}}
    \hfill
  \subfloat[d\label{1d}]{%
        \includegraphics[width=0.45\linewidth]{images/rg2cap_leader_robot_speed.eps}}
    \\
  \subfloat[d\label{1e}]{%
        \includegraphics[width=0.85\linewidth]{images/cap_ft_vs_rg_dist.eps}}
  \caption{Experimentation Results (a), (b) Trajectories of the leader and follower for Follow-The-Thread (a) and Rotate-And-Go (b) . 
        (c), (d) Speed profiles of the leader and follower for Follow-The-Thread (c) and Rotate and Go (d). 
        (e) Separation distance between robot and leader for both strategies [m].}
  \label{fig1} 
\end{figure}
